...
- [x] 一个 Docker 镜像，包含：
  ...
  - Claude SEO 项目（git submodule 形式）
...
/app/
  ...
  └── /workspace/                 # 运行时按 job_id 创建子目录
      └── {job_id}/
          ├── .claude/
          │   ├── skills/         # symlink 或 copy 自 /app/claude-seo/skills/
          │   └── agents/         # symlink 或 copy 自 /app/claude-seo/agents/
...
5. **调 SDK**：
   ...
   async for message in query(prompt=f"运行 /seo page {url}", options=options):
       collect(message)
   ```
6. **聚合输出**：从 Claude 的多轮消息里提取最终报告文本
...
| R3 | Cloudflare Containers 仍是 public beta，可能有未知限制 | 🟠 中 | 备选：Fly.io（同 Docker 镜像），Railway，Render |
