...
1. **一个可用的 Claude API 兼容端点**
   - 可以是官方 `https://api.anthropic.com`（需 Anthropic 账号 + 付款方式）
   - 可以是运维方自建的翻译代理后接 DeepSeek / Kimi / 通义千问 / GLM
   - 可以是任何符合 Anthropic API 格式的兼容服务
2. **对应的 API key**
3. **（如果用翻译代理）该代理正确处理 Claude 的 tool-use 协议**
...
| Day 3 | 把 Claude SEO 加进容器（submodule + symlink 进 /workspace/）；Agent SDK 调用 `/seo page` 跑通 |
...
**v2 移除的交付物**：v1 里的 "LiteLLM 配置示例" 因代理剥离而移除。README 里会用**单独一节**提示"如果你需要用 DeepSeek 等非 Anthropic 官方模型，请自行部署 Claude 兼容代理（建议方案：LiteLLM / one-api），本仓库不提供"。
...
- 付费 / 订阅
- 监控、日志、告警
- **可选**：提供"一键部署运维方代理"的伴随仓库（和 wrapper 仓库分开），但不是这个 MVP 的事
...
- [ ] README 里是否给出 "推荐的 endpoint 搭建教程链接"（建议给，但内容在 wrapper 仓库之外）
